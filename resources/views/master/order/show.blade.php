...
@section('content')
    <div class="container-fluid">
        ...
        @if ($order->is_approved == false)
            <button type="button" class="btn btn-success waves-effect pull-right" onclick="approveorder({{ $order->id }})">
                <i class="material-icons">done</i>
                <span>XÁC NHẬN</span>
            </button>
            <form method="POST" action="{{ route('master.order.approve',$order->id) }}" id="approval-form" style="display: none">
                @csrf
                @method('PUT')
            </form>
        @else
